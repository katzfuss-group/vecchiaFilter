...
void Lorenz_model::dZ_3( const vector &Z, vector &dZ )
{
	...
	for( int n=0; n<N; ++n )
	{
		...
		double YY = -Y[ mod(n-2) ] * Y[ mod(n-1) ] + Y[ mod(n-1) ] * Y[ mod(n+1) ];
		double YX = -Y[ mod(n-2) ] * X[ mod(n-1) ] + Y[ mod(n-1) ] * X[ mod(n+1) ];
		double XX = bracket[n];

		
		//if( n==880 ){
		//  std::cout << "[X,X] = " << XX << ", [Y,Y] = " << YY << ", [Y,X] = " << YX << ", X(" << n << ") = " << X[n] << ", Y(j) = " << Y[n] << ", F = " << F << std::endl;
		//}
	}
