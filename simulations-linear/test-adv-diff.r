...
t0 = proc.time()
covmodel = GPvecchia::getMatCov(vecchia.approx, covfun.d)
t1 = proc.time()
cat(paste((t1 - t0)[3], "\n"))
...
for (t in 2:Tmax) {
  ...
  t0 = proc.time()
  M1 = GPvecchia::getMatCov(vecchia.approx, Matrix::t(Fmat), factor = TRUE)
  t1 = proc.time()
  cat(paste((t1 - t0)[3], "\n"))
  cat("\t... from function: ")
  t0 = proc.time()
  M2 = GPvecchia::getMatCov(vecchia.approx, covfun.d)
  t1 = proc.time()
  cat(paste((t1 - t0)[3], "\n"))
  covmodel = M1 + M2
